...
Validator.prototype.in = function(options) {
    ...
    if (typeof options === 'string') {
      ...
      if (options.indexOf(this.str) === -1) {   //inverse of contains()
        return this.error(this.msg || 'Value not in string');
      }
      else return this;
    }
    
    //check this str is in the given array
    
    if (!options.some) {
       return this.error(this.msg || 'Argument is not an array');
    }
    
    var result = options.some(function(item) {
      return item == this;
    }, this.str);
    if (result !== true) {
       return this.error(this.msg || 'Value not in list');
    }
    return this;
};
