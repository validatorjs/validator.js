...
**isNationalId(str, [, countryCode [, options]])**          | check if the string is a national identification number,<br/><br/>`countryCode` is either an array of **ISO 3166-1 alpha-2** (e.g. `['PK', 'IT']`) OR one of `['AE', 'AL', 'AT', 'BA', 'BD', 'BH', 'BR', 'CL', 'CN', 'CU', 'FR', 'HK', 'HR', 'ID', 'IE', 'IN', 'IR', 'IS', 'IT', 'JP', 'KR', 'KW', 'LK', 'LU', 'ME', 'MK', 'MO', 'MY', 'PH', 'PK', 'RS', 'SG', 'SI', 'TH', 'TW', 'VN', 'XK']` OR defaults to `'any'`. If 'any' or a falsy value is used, function will check if any of the country codes match.<br/><br/>`options` is an optional object that can be supplied with the following keys: `strictMode`; if this is set to `true` and the `countryCode` is supported by checksum validator, then the identification number must satisfy the checksum validation.<br/><br/>Supported country code list is `validator.isNationalIdCountryCodes`.
