...
**isHash(str, algorithm [, options])**  | check if the string is a hash of type algorithm.<br/><br/>Algorithm is one of `['crc32', 'crc32b', 'md4', 'md5', 'ripemd128', 'ripemd160', 'sha1', 'sha256', 'sha384', 'sha512', 'tiger128', 'tiger160', 'tiger192']`.<br/><br/>`options` is an object which defaults to `{ validateAlgorithm: true }`.<br/>**Options:**<br/>`validateAlgorithm`: if `true`, an unrecognised `algorithm` throws an `Error` instead of silently returning `false`. Set it to `false` to keep the previous behaviour of returning `false`.
