...
const CH = (str) => {
  ...
  const hasValidCheckNumber = (digits) => {
    ...
    const calculatedCheckNumber = (11 - (digits.reduce((acc, el, idx) => acc + (el * weights[idx]), 0) % 11)) % 11; // eslint-disable-line max-len

    return lastDigit === calculatedCheckNumber;
  };

  // @see {@link https://www.estv.admin.ch/estv/de/home/mehrwertsteuer/uid/mwst-uid-nummer.html}
  return /^(CHE[- ]?)?(\d{9}|(\d{3}\.\d{3}\.\d{3})|(\d{3} \d{3} \d{3})) ?(TVA|MWST|IVA)?$/.test(str) && hasValidCheckNumber((str.match(/\d/g).map((el) => +el))); // eslint-disable-line max-len
};
